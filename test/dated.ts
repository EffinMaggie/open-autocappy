...
function testSort(log: LogFunction): boolean {
  const tt: Array<{name: string, have: Array<carrier>, expect: Array<string>}> = [
    { name: 'ensure correct sorting by date',
      have: [ new carrier([new Date('2020-01-01')], 'Y2k'),
    ...
    { name: 'validate expected overlap behavior',
      have: [ new carrier([new Date('2020-01-01'), new Date('2021-01-01')], 'last'),
